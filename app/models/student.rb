...
class Student < ActiveRecord::Base
  ...
  after_create :update_courses
	...
	def optional_points
		...
		self.courses.sum("ects_points") + self.sum_ects_points(self.studyplan_courses) - basic_points - project_points - main_points + self.excessed_points
	end
	
	def optional_points_passed
		self.passed_courses.sum("ects_points") - basic_points_passed - project_points_passed - main_points_passed + self.excessed_points_passed
	end
	...
	def excessed_points_passed
		points = 0.0
		points += basic_points_passed - 45.0 		if basic_points_passed > 45
		points += project_points_passed - 45.0	if project_points_passed > 45
		points += main_points_passed - 45.0 		if main_points_passed > 45
		points
	end
	
	def total_points
		self.courses.sum("ects_points") + self.sum_ects_points(self.studyplan_courses)
	end
